<div>
    <div x-data="{ slideOut: false }" @keydown.window.escape="slideOut = false" {{-- x-init="() => { $watch('slideOut', value => toggleScrollLock(value)) }" --}}>
        <button aria-label="Abrir la ventana del carrito de compras" @click="slideOut = !slideOut" class="relative flex items-center justify-center">
            ...
            @if (Cart::count())
                <span class="absolute top-0 right-0 animate-bounce inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-red-100 transform translate-x-1/2 -translate-y-1/2 bg-red-600 rounded-full">
                    {{Cart::count()}}
                </span>  
            @else
            @endif
                    ...
                    <div class="bg-white">
                        @if (Cart::count())
                            <div class="py-4 border px-2 rounded-md">
                                ...
                                <div class="mt-2">
                                    <a aria-label="Abrir el carrito de compras" href="{{ route('shopping-cart') }}" class="w-full flex justify-center items-center border border-[#60A3BD] text-[#60A3BD] hover:bg-[#60A3BD] fill-[#60A3BD] hover:fill-white rounded-full px-3 py-2 hover:text-white">
                                        <span class="mr-2">
                                ...
                                <div class="mt-2">
                                    <a aria-label="Comprar directamente" href="{{ route('orders.create') }}" class="w-full flex justify-center items-center border border-[#3E3E66] text-[#3E3E66] hover:bg-[#3E3E66] fill-[#3E3E66] hover:fill-white rounded-full px-3 py-2 hover:text-white">
                                        <span class="mr-2">
